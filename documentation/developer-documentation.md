...
The time class can calculate the current tide level given the Highest Astronomical Level (HAT). The tide is calculated based on the current time point and the phase of the moon. A normal 29.5 day lunar cycle is used to distinguish between spring and neap tides. Neap high tide is 20% lower than the HAT and 20% higher than the LAT. The LAT is 0.

The tide level and phase of the moon is deterministic, there is no need to save it.
